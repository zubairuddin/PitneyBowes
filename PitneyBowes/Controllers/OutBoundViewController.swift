...
class OutBoundViewController: UIViewController {
    ...
    @IBAction func submitOutboundInfo() {
        ...
        APIManager.saveOutboundInfo(appendingPath: "shipments/add", withQueryString: "user_id=\(userID)&type=\(type)&bol=\(generalInfo.bolNumber)&pro_number=\(generalInfo.proNumber)&carrier=\(generalInfo.carrier)&ngs_location=\(generalInfo.ngsLocation)&latitude=\(generalInfo.latitude)&longitude=\(generalInfo.longitude)&origin=\(generalInfo.destination)&state_code=\(generalInfo.stateCode)&employee_name=\(generalInfo.employeeName)&driver_name=\(driverInfo.driverName)&seal_number=\(driverInfo.sealNumber)&lock_on_trailer=\(driverInfo.lockOnTrailer)&trailer_number=\(tractorInfo.trailerNumber)&tractor_dot_number=\(tractorInfo.tractorDotNumber)&tractor_plate=\(tractorInfo.tractorPlate)&dont_discuss_shipment=\(questionaireInfo.discussShipment)&dont_carry_passengers=\(questionaireInfo.carryPassengers)&acknowledge_receipt=\(questionaireInfo.receiptAcknowledge)&alert_contact=\(questionaireInfo.alertContact)", paramerer: params) { (data, error) in
