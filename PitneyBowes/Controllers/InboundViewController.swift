...
class InboundViewController : UIViewController {
    ...
    @IBAction func submitInboundShipment(_ sender: UIButton) {
        ...
        APIManager.executeRequest(appendingPath: "shipments/add", withQueryString: "user_id=\(userId)&bol=\(generalInfo.bol)&pro_number=\(generalInfo.proNumber)&carries=\(generalInfo.carrier)&ngs_location=\(generalInfo.ngsLocation)&latitude=\(generalInfo.latitude)&longitude=\(generalInfo.longitude)&origin=\(generalInfo.origin)&employee_name=\(generalInfo.employeeName)&driver_name=\(driverInfo.driverName)&seal_number=\(driverInfo.sealNumber)&lock_on_trailer=\(driverInfo.isLockOnTrailer)", httpMethod: "POST") { (data, error) in
